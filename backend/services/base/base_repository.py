...
class BaseRepository(metaclass=SingletonMeta):
    ...
    def __init__(self):
        ...
        if is_sqlite:
            ...
            self.logger.debug(f"Pool size: {DB_POOL_SIZE}")
            self.logger.debug(f"Max overflow: {DB_MAX_OVERFLOW}")
            self.logger.debug(f"Pool timeout: {DB_POOL_TIMEOUT}")
            self.logger.debug(f"Pool recycle: {DB_POOL_RECYCLE}")

...
class TelethonService(BaseService):
    ...
    async def get_user_sent_messages(self, client: TelegramClient, limit: int = 200) -> list[dict]:
        """
        ...
        try:
            ...
            async for dialog in client.iter_dialogs(limit=50):
                if dialog.is_user or dialog.is_group:
                    ...
                    async for message in client.iter_messages(dialog, from_user='me', limit=100):
            ...
            self.logger.error(f"Error fetching user sent messages: {e}", exc_info=True)
